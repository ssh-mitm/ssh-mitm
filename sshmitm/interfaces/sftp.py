...
class SFTPProxyServerInterface(BaseSFTPServerInterface):
    ...
    def chattr(self, path: str, attr: SFTPAttributes) -> int:
        ...
        if attr.st_mode:
            return self.session.sftp.client.chmod(path, attr.st_mode)
        ...
        return self.session.sftp.client.chown(path, attr.st_uid, attr.st_gid)
    ...
    def list_folder(self, path: str) -> list[SFTPAttributes] | int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return cast("list[SFTPAttributes]", self.session.sftp.client.listdir_attr(path))
    ...
    def lstat(self, path: str) -> SFTPAttributes | int:
        ...
        if self.session.sftp.client is None:
            ...
            raise MissingClient(msg)
        try:
            return cast("SFTPAttributes", self.session.sftp.client.lstat(path))
    ...
    def mkdir(self, path: str, attr: SFTPAttributes) -> int:
        ...
        if attr.st_mode is None:
            return self.session.sftp.client.mkdir(path)
        return self.session.sftp.client.mkdir(path, attr.st_mode)
    ...
    def readlink(self, path: str) -> str | int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return cast("str", self.session.sftp.client.readlink(path))
    ...
    def remove(self, path: str) -> int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return self.session.sftp.client.remove(path)
    ...
    def rename(self, oldpath: str, newpath: str) -> int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return self.session.sftp.client.rename(oldpath, newpath)
    ...
    def rmdir(self, path: str) -> int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return self.session.sftp.client.rmdir(path)
    ...
    def stat(self, path: str) -> SFTPAttributes | int:
        ...
        if self.session.sftp.client is None:
            ...
            raise MissingClient(msg)
        try:
            return cast("SFTPAttributes", self.session.sftp.client.stat(path))
        except FileNotFoundError:
            logging.debug("File %s not found", path)
            return SFTP_NO_SUCH_FILE
    ...
    def symlink(self, target_path: str, path: str) -> int:
        ...
        if self.session.sftp.client is None:
            msg = "self.session.sftp.client is None!"
            raise MissingClient(msg)
        return self.session.sftp.client.symlink(target_path, path)
