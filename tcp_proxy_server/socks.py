...
class socksocket(socket.socket):
    ...
    def __init__(self, family=socket.AF_INET, sockettype=socket.SOCK_STREAM,
                 proto=0, *args, **kwargs):
        if sockettype not in (socket.SOCK_STREAM, socket.SOCK_DGRAM):
            msg = "Socket type must be stream or datagram, not {!r}"
            raise ValueError(msg.format(sockettype))

        super(socksocket, self).__init__(family, sockettype, proto, *args, **kwargs)
    ...
    def bind(self, *pos, **kw):
        ...
        (proxy_type, _, _, _, _, _) = self.proxy
    ...
    def sendto(self, bytedata, *args, **kwargs):
        if self.type != socket.SOCK_DGRAM:
            return super(socksocket, self).sendto(bytedata, *args, **kwargs)
        ...
        sent = super(socksocket, self).send(header.getvalue() + bytedata, *flags,
                                            **kwargs)
    ...
    def recv(self, *pos, **kw):
        bytedata, _ = self.recvfrom(*pos, **kw)
        return bytedata
    ...
    def _SOCKS5_request(self, conn, cmd, dst):
        ...
        """
        _, _, _, _, username, password = self.proxy
        ...
        try:
            ...
            if chosen_auth[1:2] == b"\x02":
                ...
                # authentication.
                writer.write(
                    b"\x01" + chr(len(username)).encode() + username + chr(len(password)).encode() + password
                )
                writer.flush()
    ...
    def _negotiate_HTTP(self, dest_addr, dest_port):
        ...
        _, addr, _, rdns, username, password = self.proxy
    ...
    def connect(self, dest_pair):
        ...
        (proxy_type, proxy_addr, proxy_port, _, _, _) = self.proxy

        # Do a minimal input check first
        if (
            not isinstance(dest_pair, (list, tuple)) or
            len(dest_pair) != 2 or
            not dest_addr or
