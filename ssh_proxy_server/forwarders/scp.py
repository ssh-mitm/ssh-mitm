...
class SCPReplaceForwarder(SCPForwarder):
    """
    Replaces a SCP transmitted file during transit
    """
    ...
    def process_data(self, traffic):
        if not self.file_buffer and not self.replacement:
            self.file_buffer = self.get_chunks(len(traffic))
            self.replacement = open(self.args.scp_replacement_file, "rb")

        try:
            traffic = self.replacement.read(next(self.file_buffer)) + b'\x00'
        except StopIteration:
            logging.info("gen stoped")
            self.file_buffer = None
    ...
    def get_chunks(self, buf_len):
        file_size = os.path.getsize(self.args.scp_replacement_file)
        buf = [buf_len] * (file_size // buf_len) + [file_size % buf_len]
        yield from buf
