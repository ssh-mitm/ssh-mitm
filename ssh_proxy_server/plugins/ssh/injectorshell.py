...
class SSHInjectableForwarder(SSHForwarder):
        ...
        try:
            while not self.session.ssh_channel.closed:
                ...
                if len(readable) == 1 and readable[0] is self.injector_sock:
                    ...
                    logging.debug("injector shell opened from %s to ('%s', %d)", str(addr), inject_host, inject_port)
    ...
    def close_session(self, channel):
        ...
        logging.debug("closing injector connection %s", self.injector_sock.getsockname())
...
class InjectorShell(threading.Thread):
    ...
    def terminate(self):
        ...
        logging.debug("injector shell %s was closed", str(self.remote))
